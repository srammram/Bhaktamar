...
class Siteprocurment extends CI_Model
{
    ...
    public function getAllCompanies($group_name) {
        ...
        $q = $this->db->get_where('inv_companies', array('group_name' => $group_name));
    ...
    public function lastidPurchaseInv(){
        $this->db->order_by('id' , 'DESC');
        $q = $this->db->get('inv_pro_purchase_invoices');
        if ($q->num_rows() > 0) {
            return $q->row('id');
        }
        return 0;
 ...
 function getAllPONumbers(){
    $q = $this->db->get_where('inv_pro_purchase_orders', array('status' => 'approved'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
...
//	$this->db->select('p.*,t.rate as purchase_tax_rate');
//	$this->db->from('products p');	
//	$this->db->join('tax_rates t','p.tax_rate=t.id','left');
//	
//	$this->db->where("(p.name LIKE '" . $term . "%' OR p.code LIKE '" . $term . "%' OR  concat(p.name, ' (', p.code, ')') LIKE '" . $term . "%')");
//	
//	$this->db->limit($limit);
//	
//	
...
//	    $this->db->select('inv_products.*,inv_categories.name as category_name,inv_price_master.price,inv_price_master.batch_no,inv_price_master.id as batch_id,inv_tax_rates.rate as tax,inv_pro_stock_master.stock_in as available_qty,inv_pro_stock_master.expiry_date');
 $this->db->select('inv_products.*,inv_categories.name as category_name,inv_price_master.price,inv_price_master.batch_no,inv_price_master.id as batch_id,inv_tax_rates.rate as tax');
	    $this->db->from('inv_products');
	    $this->db->join('inv_warehouses_products FWP', 'FWP.product_id=inv_products.id ');
	    $this->db->join('inv_categories','inv_categories.id=inv_products.category_id');
	   // $this->db->join('inv_pro_stock_master','inv_pro_stock_master.product_id=inv_products.id ');
	    $this->db->join('inv_price_master','inv_price_master.product_id=inv_products.id ');
	    $this->db->join('inv_tax_rates', 'inv_tax_rates.id=inv_products.tax_rate', 'left');
	    $this->db->where("(inv_products.name LIKE '" . $term . "%' OR inv_products.code LIKE '" . $term . "%' OR inv_products.barcode LIKE '" . $term . "%' OR  concat(inv_products.name, ' (', inv_products.code, ')') LIKE '" . $term . "%')");
	    $this->db->group_by('inv_products.id');
	    $this->db->limit(15);
	    //echo $this->db->get_compiled_select();
    ...
    public function getAll_respectiveSTOREREQUESTNUMBER(){
		$this->db->where('status', 'approved');
		$this->db->or_where('status', 'partial_complete');
		$this->db->where('to_store_id',$this->store_id);
		$q = $this->db->get('pro_store_request');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
