...
class Companies_model extends CI_Model
{
    ...
    public function deleteSupplier($id)
    {
     ...
     WHEN EXISTS (SELECT 1 FROM sramrms_purchases  WHERE supplier_id ='".$id."')
       OR EXISTS (SELECT 1 FROM sramrms_quotes WHERE supplier_id ='".$id."')
      THEN 'y'
     ELSE 'n'
     END supplier")->row();
	 $exists=$query->supplier;
	 if($exists =='y')
	 {
		$data=array('Active'=>1);
		$this->db->where('id',$id);
		$this->db->update('sramrms_companies',$data);
		return true;
	 }else{
