...
class Site extends CI_Model
{
    ...
    public function getAllCurrencies() {
        $q = $this->db->get('inv_currencies');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
		$this->db->where('currency_id',$row->id);
		$this->db->where('DATE(date)',date('Y-m-d'));
		$c = $this->db->get('currency_ex_rates');
		if($c->num_rows()>0){
		    $rate = $c->row('CUR_'.$this->Settings->default_currency);
		    $row->rate = $rate;
		}else{
		    $row->rate =0;
		}
    ...
    public function getCurrencyByCode($code) {
        $q = $this->db->get_where('inv_currencies', array('code' => $code), 1);
    ...
    public function getWarehouseByID($id) {
        $q = $this->db->get_where('inv_warehouses', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	//echo '<pre>';print_r($storeids);exit;
	return $storeids;
    }
	...
	$this->db->join('tender_type_status','payment_methods.id=tender_type_status.type_id');
    ...
    public function getCurrencyByID($id) { echo $id;
	...
	$q = $this->db->get('logos');
	if($q->num_rows()>0){
	    $logo_name = $q->row('site_logo');
	    $logo = 'assets/uploads/logos/'.$logo_name;
	    if(file_exists($logo)){
		return $logo;
	    }
	}
    ...
    function get_biller_logo(){
	$q = $this->db->get('logos');
	if($q->num_rows()>0){
	    $logo_name = $q->row('biller_logo');
	    $logo = 'assets/uploads/logos/'.$logo_name;
	    if(file_exists($logo)){
		return $logo;
	    }
    ...
    function getCurrencyDenomination($code=false){
	if($code){
	...
	$currency = array();
        if ($q->num_rows() > 0) {
	    $currency = $q->result();
	    foreach($currency as $k => $row){
		$this->db->order_by('denomination','DESC');
		$d = $this->db->get_where('currency_denominations',array('status'=>1,'currency_id'=>$row->id));
		if ($d->num_rows() > 0) {
		    $denominations = array();
		    foreach($d->result() as $kk => $den){
			$denominations[$den->denomination] = $den ;
		    }
		    array_multisort(array_keys($denominations),SORT_DESC , SORT_NATURAL| SORT_FLAG_CASE, $denominations);
		  
		    $currency[$k]->denominations = $denominations;
		}
	    }
        }
        return $currency;
    }
    function getTenderCurrencyDenomination(){
	$this->db->where('tendering_currency',1);
	$q = $this->db->get('currencies');
	$currency = array();
        if ($q->num_rows() > 0) {
	    $currency = $q->result();
    ...
    function getTenderCardTypes(){
	$q = $this->db->get_where('tender_card_types',array('status'=>1));
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
    }
    function getTenderWalletTypes(){
	$q = $this->db->get_where('tender_wallet_types',array('status'=>1));
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
    }
    public function getAllTenderCurrencies() {
	$this->db->where('tendering_currency',1);
    ...
    public function getAllSettlementCurrencies() {
	$this->db->where('settlement_currency',1);
        $q = $this->db->get('currencies');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    function getTillID(){
	$name = $_SERVER['REMOTE_ADDR'];//gethostname();
	$q = $this->db->get_where('tills',array('system_ip'=>$name));
	if($q->num_rows()>0){
	    return $q->row('id');
	}
	return false;
    }
    function getTillName(){
	$name = $_SERVER['REMOTE_ADDR'];
	$q = $this->db->get_where('tills',array('system_ip'=>$name));
	if($q->num_rows()>0){
	    return $q->row('till_name');
	}
	return false;
    }
	...
	if($this->isStore && $this->centerdb_connected){
	    if ($this->centerdb->insert("expense_categories", $data)) {
		return true;
	    }
	...
	$query = 'update '.$this->db->dbprefix('pro_stock_master').'
		set stock_in = stock_in - '.$stockout.' ,
		    stock_out = stock_out + '.$stockout.'
		where id='.$stockMasterId;
	    $this->db->query($query);
    }
	
	function updateStockMaster($product_id,$stock_out,$batch,$selling_price){ // ,$cm_id,$cate
		/* echo $product_id;
		echo "<br />";
		echo $cm_id;
		echo "<br />";  */
		...
		$this->db->where(array('product_id'=>$product_id, 'batch'=>$batch, 'selling_price'=>$selling_price));
		$this->db->order_by('id');
		$q = $this->db->get(); 
			...
			//	$piece = $this->db->get_where('recipe', array('id' =>$product_id))->row('piece');
				...
				if($stock_out_qty>0){
					...
					//	$update_qty * 5 
						...
						//if ($qu->num_rows() > 0) {
						$query = 'update srampos_pro_stock_master set stock_in = stock_in - '.$update_qty.',  stock_out = stock_out + '.$update_qty.' where id='.$row->id;
						$this->db->query($query);
    ...
    function create_store_products($storeid){
	$ch=curl_init();
	curl_setopt($ch,CURLOPT_URL,site_url('curl_process/assign_store_products/'.$storeid));
	curl_setopt($ch,CURLOPT_CONNECTTIMEOUT,2);
	curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
	$buffer = curl_exec($ch);
	curl_close($ch);
    ...
    function isCurrencyRatesUpdated(){
	$this->db->where('DATE(date)',date('Y-m-d'));
	$c = $this->db->get('currency_ex_rates');
	if($c->num_rows()>0){
	    return true;
    ...
    function getPreviousDayCurRates(){
	$this->db->order_by('id','desc');
	$this->db->limit(1);
	$c = $this->db->get('currency_ex_rates');
	if($c->num_rows()>0){
	    $date = date('Y-m-d',strtotime($c->row('date')));
	    $this->db->where('DATE(date)',$date);
	    $q = $this->db->get('currency_ex_rates');
	    if($q->num_rows()>0){
		$data['date'] = $date;
		$currency_rates = array();
		foreach($q->result() as $k => $row){
		    $currency_rates[$row->currency_code] = $row;
		}
		//p($currency_rates,1);
    ...
    function getAllCurrencieswithouRates(){
	$q = $this->db->get('currencies');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    function set_previous_exchangeRates(){
	$data = $this->getPreviousDayCurRates();
	$currency_rates = $data['currency_rates'];
	foreach($currency_rates as $k => $row){
	    unset($row->id);
	    $row->date = date('Y-m-d');
	    $this->db->insert('currency_ex_rates',$row);
	}
	return true;
    }
    function getTodayDayCurRates(){
	$data = array();
	$date = date('Y-m-d');
	$this->db->where('DATE(date)',$date);
	$q = $this->db->get('currency_ex_rates');
    ...
    function getActiveHoldnotes(){
	$q = $this->db->get_where('sales_hold_notes',array('status'=>1));
        if ($q->num_rows() > 0) {
            $data = $q->result();
            return $data;
        }
    ...
    function getDepartments(){
         $q = $this->db->get_where("categories", array('level' => 1));
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
    }
    function getCategories_byDepID($dep_id){
        $this->db->where('parent_id', $dep_id);
    ...
    function getUserAllowedDiscount(){
	$q = $this->db->get_where("users", array('id' => $this->loggedInUser));
        if ($q->num_rows() > 0) {
	    $discount['max_dis_amount'] = $q->row('max_bill_discount_amt');
	    $discount['max_dis_per'] = $q->row('max_bill_discount_per');
            return $discount;
        }else{
			$discount['max_dis_amount'] = 0;
			$discount['max_dis_per'] = 0;
            return $discount;
		}
        return FALSE;
    }
    function getBannerImg($page="admin"){
	$type = $this->Settings->banner_img;
	$q = $this->db->get_where('banner_img',array('type'=>$type,'page'=>$page));
	return $q->row('img_path');
    }
    
    function getAllSalesman(){
	$this->db->select('id,username');
	//$q = $this->db->get_where('users',array('active'=>1));
	$q = $this->db->get_where('users',array('active'=>1,'Is_sales_persons'=>1));
	return $q->result();
    }
    function getCurrentShift(){
	$this->db->where('(curtime() >= from_time OR curtime() <= to_time)');
	$q = $this->db->get('shiftmaster');
	return $q->row();
    }
    function isShiftCreated(){
	$till_id = $this->getTillID();
	$date = date('Y-m-d');
	$shift = $this->getCurrentShift();
	//$shift_start_time = $shift->from_time;
	//$shift_end_time = $shift->to_time;
	$now = date('H:i:s');
	$this->db->select();
	$this->db->from('shifts');
	//$this->db->where('DATE(date)',$date);
	$this->db->where('till_id',$till_id);
	$this->db->where('(TIME(shift_from_time) <= TIME("'.$now.'") && TIME(shift_to_time) >= TIME("'.$now.'"))');

	//$this->db->where('shift_to_time<=','TIME('.$shift_end_time.')');
	//echo $this->db->get_compiled_select();exit;
	$q = $this->db->get();
	if($q->num_rows()>0){
	    $data = $q->row();
	    $data->shift_name = $this->getShiftName($data->shift_id);
	    return $data;
	}else{
	    $this->db->select();
	    $this->db->from('shifts');
		...
		$continued_shift =  $q->row('continued_shift');
