...
<div class="box box-primary">
    ...
    <!-- Content Start-->
    <!-- form start -->
    <?php echo form_open_multipart('admin/payroll/employee/save_employee_personal_info', $attribute= array('id' => 'personalForm')) ?>
    <div class="box-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="marital_status">
                                ...
                                <option value="Single" <?php if(!empty($employee->marital_status)) echo $employee->marital_status == 'Single'?'selected':''  ?>><?= lang('singel') ?></option>
								...
								<option value="Divorced" <?php if(!empty($employee->marital_status)) echo $employee->marital_status == 'Divorced'?'selected':''  ?>><?= lang('Divorced') ?></option>
								 <option value="Widowed" <?php if(!empty($employee->marital_status)) echo $employee->marital_status == 'Widowed'?'selected':''  ?>><?= lang('Widowed') ?></option>
			                    <option value="Separated" <?php if(!empty($employee->marital_status)) echo $employee->marital_status == 'Separated'?'selected':''  ?>><?= lang('Separated') ?></option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="blood_group">
								...
								<option value="O-positive">O-positive</option>
											<option value="O-negative"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'O-negative'?'selected':''  ?>>O-negative</option>
											<option value="A-positive"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'A-positive'?'selected':''  ?>> A-positive</option>
											<option value="A-negative"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'A-negative'?'selected':''  ?>>A-negative</option>
											<option value="B-positive"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'B-positive'?'selected':''  ?>>B-positive</option>
											<option value="B-negative"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'B-negative'?'selected':''  ?>>B-negative</option>
											<option value="AB-positive"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'AB-positive'?'selected':''  ?>>AB-positive</option>
											<option value="AB-negative"<?php if(!empty($employee->blood_group)) echo $employee->blood_group == 'AB-negative'?'selected':''  ?>>AB-negative</option>
                            </select>
                        </div>
                    </div>
								 ...
								 <div class="col-md-6">
                                    <div class="form-group">
                                        <label><?= lang('Employee_type') ?><span class="required" aria-required="true">*</span></label>
                                        <select class="form-control select2" name="EmployeeType_id" >
                                            <option value=""><?= lang('please_select') ?>..</option>
                                            <?php foreach($employee_type as $item){ ?>
                                               <option value="<?php echo $item->Employee_Type_id ?>" <?php if(!empty($employee->EmployeeType_id)) echo $employee->EmployeeType_id == $item->Employee_Type_id ?'selected':'' ?>><?php echo $item->Name ?></option>
                                        ...
                                        <select class="form-control select2" name="Category_id">
                                            <option value=""><?= lang('please_select') ?>..</option>
                                             <?php foreach($category_settings as $item){ ?>
												 ...
												 <option value="<?php echo $item->id ?>" <?php if(!empty($employee->Category_id)) echo $employee->Category_id == $item->id ?'selected':''  ?>><?php echo $item->Categoryname ?></option>
...
<!--                                        <td><input name="personalAttach[]" value="--><?php //echo $employee->id.'_'.$key ?><!--" class="child_present" type="checkbox" /></td>-->
                                       ...
                                       <!-- <td><?php echo $item->file ?></td>-->
                                        ...
                                        <td><?php echo $item->added_by ?></td>
										 ...
										 <a  href=" <?php 
                         echo base_url().UPLOAD_EMPLOYEE.$employee->employee_id.'/'.$item->file ;

										 ?>" class="btn btn-xs btn-default  filetype"  id="<?php echo site_url('admin/employee/View_file/'.$employee->id.'_'.$key)?>"> <i class="glyphicon glyphicon-search"></i></a>
										 
				  <a style="margin-left:12px;" href="<?php echo site_url('admin/employee/download_personalAttachment/'.$employee->id.'_'.$key)?>" class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-download-alt"></i></a></div></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
