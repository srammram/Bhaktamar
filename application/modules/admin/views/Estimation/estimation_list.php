...
$(document).ready(function() {
    ...
    oTable = $('#UnitTable').dataTable({
        ...
        'bProcessing': true,
        'bServerSide': true,
        'sAjaxSource': "<?php echo  base_url()  ?>" + 'admin/Estimation/getEstimation',
    ...
    return status;
        ...
        "mRender": function (data, type, full) {
            ...
            actions += '<div class="btn-group"><a href='+"<?php echo base_url('admin/Estimation/estimation_view/'); ?>" +'/'+link+'  class="tip" ><i class=\"fa fa-eye\"></i></a> ';
        actions += '</div>';
		 }else{
			 var actions = '<div class=\"text-center\">';
            actions += '<div class="btn-group">';
			  actions += '<div class="btn-group"><a href='+"<?php echo base_url('admin/Estimation/estimation_view/'); ?>" +'/'+link+'  class="tip" ><i class=\"fa fa-eye\"></i></a> ';
			    actions += '<div class="btn-group"><a href='+"<?php echo base_url('admin/Estimation/form/'); ?>" +'/'+link+'  class="tip" ><i class=\"fa fa-edit\"></i></a> ';
        actions += '</div></div>';
		 }
    return actions;
        },}]
...
<div class="row bg-title">
        ...
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
            <h4 class="page-title"><i class="icon-layers"></i> Estimation</h4>
        </div>
        <!-- /.page title -->
        <!-- .breadcrumb -->
       ...
       <div class="col-md-12">
        ...
        <div class="col-md-3">
            <div class="white-box bg-success p-t-10 p-b-10">
                <h3 class="box-title text-white">Completed Estimation</h3>
                <ul class="list-inline two-part">
                    <li><i class="fa fa-shopping-bag text-white"></i></li>
                    <li class="text-right"><span id="completedProjects" class="counter text-white">
					<?php  $task=$this->db->get_where('estimation',array('status'=>lang('Approved'),'soft_delete'=>0))->result();
                        if(!empty($task)){  echo count($task) ;}else{ echo 0; }
                    ?>
					</span></li>
                </ul>
        ...
        <div class="col-md-3">
            <div class="white-box p-t-10 p-b-10 bg-info">
                <h3 class="box-title text-white">In Process Estimation</h3>
                <ul class="list-inline two-part">
                    <li><i class="fa fa-shopping-bag text-white"></i></li>
                    <li class="text-right"><span id="inProcessProjects" class="counter text-white">
                    <?php  $task=$this->db->get_where('estimation',array('status'=>lang('in_process'),'soft_delete'=>0))->result();
                        if(!empty($task)){  echo count($task) ;}else{ echo 0; }
                        ?>
					</span></li>
                </ul>
        ...
        <div class="col-md-3">
            <div class="white-box p-t-10 p-b-10 bg-danger">
                <h3 class="box-title text-white">Declined Estimation</h3>
                <ul class="list-inline two-part">
                    <li><i class="fa fa-shopping-bag text-white"></i></li>
                    <li class="text-right"><span id="overdueProjects" class="counter text-white">
					<?php  $task=$this->db->get_where('estimation',array('status'=>lang('decline'),'soft_delete'=>0))->result();
                    if(!empty($task)){  echo count($task) ;}else{ echo 0; }
                        ?>
					</span></li>
                </ul>
			...
			<div class="row">
				<div class="col-sm-12">
					<div class="form-group custom-action">
						<a href="<?php echo base_url('admin/Estimation/form'); ?>" class="btn btn-outline btn-success btn-sm"> New Estimation <i class="fa fa-plus" aria-hidden="true"></i></a>
