...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="<?php echo site_url('admin/procurment/products/add') ?>"> <?php echo lang('products')?>
            </a></li>
            ...
            <div class="box">
			 <div class="box-header">
              ...
              <?php
                ...
                echo form_open_multipart("admin/procurment/products/edit/" . $product->id, $attrib)
                ?>
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        ...
                        <?= form_input('name', (isset($_POST['name']) ? $_POST['name'] : ($product ? $product->name : '')), 'class="form-control gen_slug" id="name" '); ?>
                    </div>
                    <div class="form-group all">
					...
					<div class="form-group all">
                        ...
                        <?= form_input('barcode', (isset($_POST['barcode']) ? $_POST['barcode'] : ($product ? $product->barcode : '')), 'class="form-control tip" id="barcode"'); ?>
                    </div>
                    ...
                    <div class="form-group all">
                        ...
                        echo form_dropdown('category', $cat, (isset($_POST['category']) ? $_POST['category'] : ($product ? $product->category_id : '')), 'class="form-control select" id="select-category" placeholder="' . lang("select") . " " . lang("category") . '"  style="width:100%"')
                        ?>
                    </div>
                    <div class="form-group all">
                        <?= lang("subcategory", "subcategory") ?>
                        <a  style="margin-left:12px" href="<?php echo base_url('system_settings/add_category?product&sub'); ?>" data-toggle="modal" data-target="#myModal"  data-backdrop="static" data-keyboard="false">
                                <i class="fa fa-plus"></i> 
                            </a>
                    ...
                    <div class="form-group standard">
                        ...
                        <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->sma->formatDecimal($product->cost) : '')), 'class="form-control decimalcost tip" id="cost" ') ?>
                    </div>
                    <div class="form-group all">
                        <?= lang("product_price", "price") ?>
                        <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->sma->formatDecimal($product->price) : '')), 'class="form-control decimalcost tip" id="price" ') ?>
                    </div>
                    <div class="form-group all">
                        <?= lang("product_mrp", "mrp") ?>
                        <?= form_input('mrp', (isset($_POST['mrp']) ? $_POST['mrp'] : ($product ? $this->sma->formatDecimal($product->mrp) : '')), 'class="form-control decimalcost tip" id="mrp" required="required"') ?>
                    </div>
                    ...
                    <div class="form-group all">
                        ...
                        <input id="product_image" type="file" data-browse-label="<?= lang('browse'); ?>" name="product_image" data-show-upload="false"
							   ...
							   <img src="<?php  echo file_exists(base_url().$pathupload.$product->image) ? base_url().$pathupload.$product->image : base_url().$pathupload.$product->image;   ?>
							...
							<div class="col-md-12">
								<div class="form-group all">
									...
									<?php /* echo '<pre>';
										print_r($Settings);
										die; */ ?>
										<input type="radio" value="0"  id="batch_required-switch-left" class="switch_left skip" name="batch_required"  
										<?php  echo ($product->batch_required==0) ? "checked" : ''; ?> checked >
										<label for="batch_required-switch-left">No</label>
										<input type="radio" value="1" id="batch_required-switch-right" class="switch_right skip" name="batch_required" 
										<?php echo ($product->batch_required==1) ? "checked" : ''; ?>  >
						...
						<div class="col-md-12">	
							<div class="col-md-12">
								...
								<label class="control-label" for="expiry_date_required" ><?= lang("expiry_date_required"); ?></label>
										...
										name="expiry_date_required" <?php echo ($product->expiry_date_required==0) ? "checked" : ''; ?> checked >
										...
										name="expiry_date_required" <?php  echo ($product->expiry_date_required==1) ? "checked" : ''; ?>>
                ...
                <div class="col-md-6 col-md-offset-1">
                    <div class="form-group all">
                        <div class="input-group"><label><?=lang('assign_stores')?>*</label></div>
                        <div class="input-group">
                               <!-- <label><input type="checkbox" name="" value="" class="select-all">&nbsp;<?=lang('select_all')?></label>-->
                        </div>
                        <?php
                            for($i=0; $i<count($warehouses_products); $i++){
                                    $warehouse_id[] = $warehouses_products[$i]->id;
                            }
                            $wh[''] = '';
                            foreach ($warehouses as $warehouse) {
                                $check = (in_array($warehouse['id'], $warehouse_id))?'checked="checked"':false;?>
                            <div class="input-group">
                                <label><input type="checkbox" name="stores[]" value="<?=$warehouse['id']?>" <?=$check?> class="select-store">&nbsp;<?=$warehouse['name']?></label>
                                </div>
                           <?php }                                                    
                            ?>
                    </div>
                    ...
                    <div class="form-group standard">
						...
						.table_t_s .select2-container .select2-choice{margin:0px;}
						</style>
				    <div class="form-group ">
                    <div class="row" id="attribute-con">
						<table class="table_t_s" style="width:100%;border:none;">
					  <tr>
					  <th class="col-xs-6"><?= lang('Attributes'); ?></th>
					  <th class="col-xs-6"><?= lang('Properties'); ?></th>
					  </tr>
					<?php
					if(isset($productattributes)){ foreach($productattributes as $productattribute) {  ?>
					<tr>
					<td class="col-xs-6" ><?php echo $productattribute->name ; ?><input type="hidden" name="productattribute[]" value="<?php  echo  $productattribute->id;  ?>"></td>
					<td class="col-xs-6">
					<?php
					$attributevales=json_decode($product->Attribute_value);
					$result = $this->db->get_where('attribute_properties', array('Attribute_id' =>$productattribute->id )); 
					$options = array('' => 'Select One...');
					foreach($result->result() as $item) {
						$options[$item->id] = $item->Name;
						if(!empty($attributevales)){
					    for($i=0;$i<count($attributevales);$i++){
						foreach($attributevales[$i] as $key=>$itematribute){
							if($item->id == $key){
							 	$select=$item->id;
							}
						}
				   ...
				   echo form_dropdown('attribute_properties[]', $options,isset($select) ? $select : 'no', 'class="form-control tip col-md-12" id="attribute"  style="width:100%;"');
                 }
					?>
					</tr>
					<?php
                 ...
                 <!--   <div class="form-group">
                    ...
                    <div class="form-group">
					   ...
					   <div class="form-group">
                        <input name="TransactionActive" type="checkbox"  class="checkbox" id="TransactionActive" value="1" <?= empty($product->Trans_active) ? '' : 'checked="checked"' ?>/>
...
-->
                    <div class="form-group">
                        <?php echo form_submit('edit_product', $this->lang->line("edit_product"), 'class="update-product btn btn-primary"'); ?>
                    </div>
...
</section><script src="<?php echo base_url('assets/procurement/js/select2.min.js')?>" type="text/javascript"></script>	
