...
.dashboard_sec .payment_section .form-control{border:1px solid #ddd;}
.dashboard_sec .payment_section select option{display:block;}
.dashboard_sec .payment_section select{    appearance: block !important;    -moz-appearance: block!important;
...
<div class="row">
<form action="<?php echo site_url('admin/Office/Check_out_save/'); ?>" role="form" id="Booking_form" autocomplete="off" enctype="multipart/form-data" method="post" accept-charset="utf-8" >
<input type="hidden" value="<?php  if(isset($id)){  echo $id ; } ?>" name="bookid">
<input type="hidden" value="<?php if(!empty($BookingDetails->Guest_id)) echo $BookingDetails->Guest_id ;  ?>" name="guestid">
...
<select name="Booking_mode" class="form-control" id="Booking_type"  tabindex="-1" title="Booking type">
<option value="<?php echo lang('Checkins');?>" <?php if(!empty($BookingDetails->Booking_mode)) echo $BookingDetails->Booking_mode ==    lang('Checkins') ? 'selected':''   ?>><?php echo lang('Checkins');?></option>
<option value="<?php echo lang('bookings');?>" <?php if(!empty($BookingDetails->Booking_mode)) echo $BookingDetails->Booking_mode ==    lang('bookings') ? 'selected':''   ?>><?php echo lang('bookings');?></option>
	...
	<div class="col-md-4">
							...
							<option value="<?php echo lang('Reservation');?>" <?php if(!empty($BookingDetails->reservation_type)) echo $BookingDetails->reservation_type ==    lang('Reservation') ? 'selected':''   ?>><?php echo lang('Reservation');?></option>
							<option value="<?php echo lang('Confirmed');?>"<?php if(!empty($BookingDetails->reservation_type)) echo $BookingDetails->reservation_type ==    lang('Confirmed') ? 'selected':''   ?>><?php echo lang('Confirmed');?></option>
							<option value="<?php echo lang('Check_in');?>"<?php if(!empty($BookingDetails->reservation_type)) echo $BookingDetails->reservation_type ==    lang('Check_in') ? 'selected':''   ?>><?php echo lang('Check_in');?></option>
							</select>
							</div>
							</div>
<div class="clearfix"></div>
...
<?php
 if(isset($getunit)){
	 $total=array();
	 foreach($getunit as $getunits){
		 $total[] =($getunits->Price)+($getunits->Extra_price);

	 }
 }
?>
<input type="hidden" class="unittotal" value="<?php echo array_sum($total);  ?>">
...
<legend><?php  echo lang('ServicesChargeDetails'); ?></legend>
	<div class="row">
		<div style="clear:both; height:15px;"></div>
<div>

<table class="tabel" style="width:100%;">
	<tbody class="servicesbody">
		<tr >
			<td>
			<div class="col-sm-12">
				<div class="form-group" style="margin-bottom: 5px;">
					<label><?php echo lang('Services_name');  ?></label>
					<select class="form-control" name="servicesname[]">
						<option value="<?php  echo lang('Laundry')  ;  ?>"><?php  echo lang('Laundry')  ;  ?></option>
						<option value="<?php  echo lang('Cart_Service')  ;  ?>"><?php  echo lang('Cart_Service')  ;  ?></option>
						<option value="<?php  echo lang('Snak_Bar_Service')  ;  ?>"><?php  echo lang('Snak_Bar_Service')  ;  ?></option>
						<option value="<?php  echo lang('Room_Service')  ;  ?>"><?php  echo lang('Room_Service')  ;  ?></option>
			...
			<td>
				<div class="col-sm-12">
					<div class="form-group" style="margin-bottom: 5px;">
					<label><?php echo lang('ServiceCharges');  ?></label>
					<input type="text" class="pa form-control  amount valid servicesprices" name="servicesprices[]" autocomplete="off" aria-invalid="false">
					</div>
				</div>
			</td>
			
			<td><span style="width:30px;height:30px;line-height:30px;border:1px solid #999;    position: relative;float: left;    border-radius: 50px;text-align: center;    margin-top: 17px;"><i class="fa fa-plus"></i></span></td>
			
		</tr>
	</tbody>
</table>
	<input type="hidden" id="servicestotal" value="">
	<div style="clear:both; height:15px;"></div>
</div>
	</div>
</fieldset>
...
<div class="col-md-12">
<div class="form-group">
<input type="submit" value="<?php  echo lang('Checkout'); ?>" class="btn btn-success" id="payment">
</div>
</div>                   
</form>                                
</div>
...
$(document).ready(function(){
	...
	var wrapper = $('.servicesbody'); 
	var fieldHTML = '<tr><td><div class="col-sm-12"><div class="form-group" style="margin-bottom: 5px;"><label></label><select class="form-control" name="servicesname[]"><option value="<?php  echo lang('Laundry')  ;  ?>"><?php  echo lang('Laundry')  ;  ?></option><option value="<?php  echo lang('Cart_Service')  ;  ?>"><?php  echo lang('Cart_Service')  ;  ?></option><option value="<?php  echo lang('Snak_Bar_Service')  ;  ?>"><?php  echo lang('Snak_Bar_Service')  ;  ?></option><option value="<?php  echo lang('Room_Service')  ;  ?>"><?php  echo lang('Room_Service')  ;  ?></option><option value="<?php  echo lang('others')  ;  ?>"><?php  echo lang('Others')  ;  ?></option></select></div></div></td><td><div class="col-sm-12"><div class="form-group" style="margin-bottom: 5px;"><label></label><input name="description[]"type="text" class="form-control"></div></div></td><td><div class="col-sm-12"><div class="form-group" style="margin-bottom: 5px;" ><label></label><input type="text" class="pa form-control  amount valid servicesprices" name="servicesprices[]" autocomplete="off" aria-invalid="false"></div></div></td><td><span style="width:30px;height:30px;line-height:30px;border:1px solid #999;    position: relative;float: left;    border-radius: 50px;text-align: center;    margin-top: 17px;"><i class="glyphicon glyphicon-remove"></i></span></td></tr> '; 
	var x = 1; 
...
<script>
$(document).on('change', '.servicesprices', function(){
    var servicetotal = 0; 
    $('.servicesprices').each(function() {
        servicetotal +=  Number(isNaN($(this).val()) ? 0 : $(this).val())
    });
	$('#servicestotal').val(servicetotal);
});
</script>
<script>
$(document).on('change', '.servicesprices', function(){
