    ...
    /*     $('#pdf').click(function (event) {
      ...
      /*  $('#image').click(function (event) {
        ...
        $(document).on('click', '.pagination a',function(e){
            e.preventDefault();
            $url = '<?php echo  base_url('admin/reports/get_payment_reports')?>';
            $url_seg = $url.split('/');
                        ...
                        success: function (data) {
                            ...
                            $('#ReportDetailsData > tbody').append('<tr><td colspan="9" class="dataTables_empty">No data available in table</td></tr>');    
                                ...
                                $.each(data.reports, function (a,b) 
                                {
                                      ...
                                      $html +='<td>'+b.floor+'</td>';
									  $html +='<td>'+b.name+'</td>';
									   $html +='<td>'+ b.bpm +'('+ b.percetage+'%)'+'</td>';
									  $html +='<td>'+ tt +formatMoney(b.amount)+'</td>';
                                     if (b.paid_status ==0){
										  $html +='<td><span class="label label-success">Paid</span></td>';
									 }
								     if (b.paid_status ==1){
								     $html +='<td><span class="label label-danger">Unpaid</span></td>';
									}
								    if (b.paid_status ==2){
										$html +='<td><span class="label label-danger">Due</span></td>';
								}	
                                      $html +='</tr>';
...
$("#xls").click(function(){
      $("#ReportDetailsData").table2excel({
        exclude: ".noExl",
        name: "Worksheet Name",
        filename: "Payment_report  " //do not include extension
