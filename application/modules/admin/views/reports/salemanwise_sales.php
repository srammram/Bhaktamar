...
<script type="text/javascript">
    ...
    /*     $('#pdf').click(function (event) {
        ...
        $(document).on('click', '.pagination a',function(e){
            e.preventDefault();
            $url = '<?php echo  base_url('admin/reports/get_salemanwiseSales_reports')?>';
            $url_seg = $url.split('/');
    ...
    if (start !='' ) {                  
        $('#start_date,#end_date').css('border-color', '#ccc'); 
                  $.ajax({
                        type: 'POST',
                        url: $url,
                        data: {start: start, end: end,pagelimit:pagelimit,agenttype:agenttype,name:name},
                        dataType: "json",
                        success: function (data) {
                            ...
                            $('#ReportDetailsData > tbody').append('<tr><td colspan="10" class="dataTables_empty">No data available in table</td></tr>');    
                                ...
                                $.each(data.reports, function (a,b) 
                                {
									  ...
									  $html +='<td>'+ currencysymbol +b.total_cost+'</td>';
                                      $html +='</tr>';
...
$("#xls").click(function(){
      ...
      $("#ReportDetailsData").table2excel({
        exclude: ".noExl",
        name: "Worksheet Name",
        filename: "SalesmanwiseSalesReport  " //do not include extension
