...
class Purchase_invoices extends Admin_Controller
{
    ...
    public function __construct()
    {
     ...
     /*    if (!$this->loggedIn) {
       ...
       // $this->lang->load('procurment/purchase_invoices', $this->Settings->user_language);
        $this->load->library('form_validation');
        $this->load->model('procurment/purchase_invoices_model');
	//	$this->load->model('Siteprocurment');
    ...
    public function index($warehouse_id = null)
    {
       ...
       /*  $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('purchase_invoices')));
        ...
        $this->render_admin('procurment/purchase_invoices/index', $data);
    }
    ...
    {     
        ...
        $view_link = '<a href="'.base_url('admin/procurment/purchase_invoices/view/$1').'" data-toggle="modal" data-target="#myModal"><i class="fa fa-edit"></i>'.lang('view_purchase_invoice').'</a>';
        ...
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . base_url('admin/procurment/purchase_invoices/delete/$1') . "'>"
...
/*         if ($warehouse_id) {        
            ...
            $this->datatables
                ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, invoice_no, supplier, total, total_discount, total_tax, shipping, round_off, grand_total, status") // ,attachment
                ->from('pro_purchase_invoices')
                ->where('warehouse_id', $warehouse_id);  // po_number,
        } else { */
            // echo "sdsd";exit;
            $this->datatables
                 ->select("id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, invoice_no, supplier, total, total_discount, total_tax, shipping, round_off, grand_total, status")
                ->from('inv_pro_purchase_invoices'); // po_number, ,attachment
...
//generate pdf and force to download
    ...
    /* -------------------------------------------------------------------------------------------------------------------------------- */
     public function add($purchase_invoices_id = null){
		...
		$store_id = $data['default_store']; 
        if ($this->form_validation->run() == true) {  
	    $n = $this->siteprocurment->lastidPurchaseInv();
	    $reference = 'PI'.str_pad($n + 1, 5, 0, STR_PAD_LEFT);
            $warehouse_id = $this->input->post('warehouse');
            $status = $this->input->post('status');                      
            $supplier_details = $this->purchase_invoices_model->getCompanyByID($this->input->post('supplier'));
            ...
            $data = array(
				...
				'sub_total' => ($this->input->post('sub_total'))?$this->input->post('sub_total'):0,   
                'total_tax' => $this->input->post('tax'),
                'grand_total' => ($this->input->post('net_amt'))?$this->input->post('net_amt'):0,               
	    ...
	    if(isset($_POST['product'])){
			...
			for($i=0;$i<$p_count;$i++){
				...
				$items[$i]['expiry'] = $this->input->post('expiry['.$i.']') ? $this->input->post('expiry['.$i.']') : '';
				$items[$i]['expiry_type'] = $this->input->post('expiry_type['.$i.']') ? $this->input->post('expiry_type['.$i.']') : '';
			...
			@	$items[$i]['category_id'] = $_POST['category_id'][$i] ? $_POST['category_id'][$i] : 0;
			@	$items[$i]['category_name'] = $_POST['category_name'][$i] ? $_POST['category_name'][$i] : '';
			@	$items[$i]['subcategory_id'] = $_POST['subcategory_id'][$i] ? $_POST['subcategory_id'][$i] : 0;
			@	$items[$i]['subcategory_name'] = $_POST['subcategory_name'][$i] ? $_POST['subcategory_name'][$i] : '';
		    @		$items[$i]['brand_id'] = $_POST['brand_id'][$i] ? $_POST['brand_id'][$i] : 0;
			@	$items[$i]['brand_name'] = $_POST['brand_name'][$i] ? $_POST['brand_name'][$i] : '';
        ...
        if ($this->form_validation->run() == true && $this->purchase_invoices_model->addPurchase_invoices($data,$items,$po_array)) {
            ...
            redirect('admin/procurment/purchase_invoices');
        } else {
            $data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $data['suppliers'] = $this->siteprocurment->getAllCompanies('supplier');
            $data['categories'] = $this->siteprocurment->getAllCategories();
            $data['currencies'] = $this->siteprocurment->getAllCurrencies();
            $data['tax_rates'] = $this->siteprocurment->getAllTaxRates();
            $data['warehouses'] = $this->siteprocurment->getAllWarehouses();
            $data['purchaseorder'] = $this->siteprocurment->getAllPONumbers(); 
			$data['Settings'] = $this->siteprocurment->getSettings(); 
           @ $this->render_admin('procurment/purchase_invoices/add', $data);
        }
    }
    public function purchase_orders_list(){
	   $poref =  $this->input->get('poref');
		...
		/* print_r($data['purchase_invoices']);
		die; */
	...
	// echo '<pre>';print_R($inv_items);exit;
	    ...
	    foreach ($inv_items as $item) {
			...
			$row->category_id = $item->category_id ? $item->category_id : 0;
			$row->category_name = $item->category_name ? $item->category_name : '';
			$row->subcategory_id = $item->subcategory_id ? $item->subcategory_id : 0;
			$row->subcategory_name = $item->subcategory_name ? $item->subcategory_name : '';
			$row->brand_id = $item->brand_id ? $item->brand_id : 0;
			$row->brand_name = $item->brand_name ? $item->brand_name : '';
			...
			$options = $this->purchase_invoices_model->getProductOptions($row->id);
                ...
                $pr[$item_key] = array('id' => $c,'store_id'=>$item->store_id,'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",
                    'row' => $row, 'tax_rate_id' => $item->tax_rate_id,'tax_rate_val' => $row->tax_rate_val,'tax_rate' => $item->item_tax, 'units' => $units, 'options' => $options);  // 
                $c++;
        }
...
/*common_quote_items*/
	...
	//$this->data['default_store'] = $this->store_id;
	//$store_id = $this->data['default_store'];
    // $this->session->unset_userdata('csrf_token');
	 $data['inv'] = $this->purchase_invoices_model->getPurchase_invoicesByID($id);
	
	
	if ($data['inv']->status == 'approved' || $data['inv']->status == 'completed') {
		$this->session->set_flashdata('error', lang("Do_not_allowed_edit_option"));
		redirect("admin/procurment/purchase_invoices");
	}
	      ...
	      $supplier_details = $this->siteprocurment->getCompanyByID($this->input->post('supplier'));
           $supplier = $supplier_details->name;
            $data = array(
				...
				'sub_total' => $this->input->post('sub_total'),   
                'total_tax' => $this->input->post('tax'),
                'grand_total' => $this->input->post('bill_net_amt'),                
        ...
        if ($this->form_validation->run() == true && $this->purchase_invoices_model->updatePurchase_invoices($id,$data,$items,$po_array)) {
			...
			$data['purchaseorder'] = $this->siteprocurment->getAllPONumbers();
            $data['Settings'] = $this->siteprocurment->getSettings();
            $data['inv_items'] = $this->purchase_invoices_model->getAllPurchase_invoicesItems($id);
			
	    $c=1;
		$settings=$this->siteprocurment->getSettings();
        ...
        $options = $this->purchase_invoices_model->getProductOptions($row->id);
        $units = $this->siteprocurment->getUnitsByBUID($row->base_unit);
        $ri = $settings->item_addition ? $row->id : $row->id;
...
//echo '<pre>';print_r($this->data['inv']);exit;
            $data['json_inv_items'] = json_encode($pr);
           $this->render_admin('procurment/purchase_invoices/edit', $data);
        }
    }
   ...
   public function delete($id = null)
    ...
    public function suggestions()
    {
        ...
        if (strlen($term) < 1 || !$term) {
            die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . base_url('admin/procurment/welcome') . "'; }, 10);</script>");
        }
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                $row->item_tax_rate = '0';
		 $tax_rate = $this->siteprocurment->getTaxRateByID($row->tax_rate);
		$row->purchase_tax_rate = $tax_rate->rate;
