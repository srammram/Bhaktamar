...
class Users extends Admin_Controller{
    ...
    function index(){
		...
		$this->render_admin('users/list', $data);		
	}
	function get_users(){
		    $actions = "<div class=\"text-center\">";
            $actions .= " <a href='" . base_url('admin/Users/form/$1') . "'  class='tip'   ><i class=\"fa fa-edit\"></i></a> <a href='" . base_url('admin/Users/delete/$1') . "' class='tip po'  onclick='return areyousure(this)'><i class=\"fa fa-trash-o\"></i></a>";
            $actions .= "</div>";
		    $this->load->library('datatables');
		    $this->datatables
		   ->select(" id , username,first_name, email", FALSE)
		   ->from("users")
		  // ->where("soft_delete",0)
	...
	function view($id,$tab=false){
		...
		$data['page_title']	= lang('view')." ".lang('Users') ;
		$this->render_admin('users/view', $data);
	}
	function form($id = false){
		$data['page_title']		        = lang('add_Users');
		...
		if ($this->form_validation->run() == FALSE){
			$this->render_admin('users/form', $data);		
		}else{
			 ...
			 if($id){
				...
				$this->session->set_flashdata('message', lang('Users_saved'));
			 }
			redirect('admin/Users');
		}
	}
	
	function userdelete($id = false){
		if ($id){	
			$user	= $this->setting_model->getUsersByid($id);
			if (!$user){
				$this->session->set_flashdata('error', lang('Users_details_not_found'));
				redirect('admin/Users');
			}else{
				...
				$this->session->set_flashdata('message', lang('Users_details_deleted'));
				redirect('admin/Users');
			}
		}else{
			    $this->session->set_flashdata('error', lang('Users_details_not_found'));
    ...
    function getUsers()
    {
        ...
        $actions = "<div class=\"text-center\">";
        if($this->isWarehouse){
            $actions .= "<a href='" . admin_url('auth/profile/$1') . "' class='tip' title='" . lang("edit_user") . "'><i class=\"fa fa-edit\"></i></a>";
        }else if(isStore){
            $actions .= "<a href='" . admin_url('auth/view_profile/$1') . "' class='tip' title='" . lang("view_user") . "'><i class=\"fa fa-eye\"></i></a>";
        }
        $actions .= "</div>";
        $this->load->library('datatables');
        $this->datatables
            ->select($this->db->dbprefix('users').".id as id, first_name, last_name, email,  award_points, " . $this->db->dbprefix('groups') . ".name, active")
            ->from("users")
...
//->where('company_id', NULL)
    ...
    function view_profile($id){
        $this->sma->checkPermissions('view', true);
        $user = $this->ion_auth->user($id)->row();
        $groups = $this->ion_auth->groups()->result_array();
        $this->data['csrf'] = $this->_get_csrf_nonce();
        $this->data['user'] = $user;
        ...
        $meta = array('page_title' => lang('profile'), 'bc' => $bc);
        $this->data['stores'] = $this->site->getAllStores();
        $this->page_construct('auth/view_profile', $meta, $this->data);
    }
    ...
    function create_user()
    {
		...
		'Is_sales_persons'=> $this->input->post('is_salesperson'),
                'max_bill_discount_per' => $this->input->post('max_bill_discount_per'),
                'max_bill_discount_amt' => $this->input->post('max_bill_discount_amt'),
            );
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $user->group);
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $user->status);
                        $row++;
                    }
    ...
    function delete($id = NULL)
    {
        ...
        if ( ! $this->Owner || $id == $this->session->userdata('user_id')) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect(isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'admin/dashboard');
        }
