<?php 
 $Owner = $this->session->userdata('owner');
		$this->db->select("owner_unit_request.id,owner_unit_request.request_status,unit_name,project.Name project,floors.name as floors,building_info.name building,requesteddate,title,request_type,owner_realtion_unitid,view_link,owner_accept,is_closed");
					...
					<ul class="dropdown-menu" id="notification_menu_sec">
						<?php if($totalRequest) { foreach($request as $item){ ?>
							...
							<a href="<?php echo base_url('owner/Myunits/'.$item->view_link).'/'.$item->id ?>">
										...
										<?php    } else{  ?>
										<span style="color:red;">IsClosed</span>
										<?php   }  ?>
										</a>
							</li>
							<?php } }else{ ?>
            ...
            <ul class="dropdown-menu">
              ...
              <li class="user-footer">
                ...
                <div class="pull-right">
                  <a href="<?php echo site_url('owner/login/logout')?>" class="btn btn-default btn-flat"><?php echo lang('signout')?></a>
                </div>
              </li>
            </ul>
          </li>
