    ...
    public function attendance_excel_import($file)
    {
        ...
        foreach ($excel->getWorksheetIterator() as $worksheet)
        {
            ...
            for($row=2; $row<=$highestRow; $row++)
            {
				...
				$Clock_in		= ($worksheet->getCellByColumnAndRow(9, $row)->getValue()!=NULL) ? date("H:i:s", strtotime($worksheet->getCellByColumnAndRow(9, $row)->getValue())) : '';
				$Clock_out  	= ($worksheet->getCellByColumnAndRow(10, $row)->getValue()!=NULL) ? date("H:i:s", strtotime($worksheet->getCellByColumnAndRow(10, $row)->getValue())) : '';
				...
				$result = $CI->db->get_where('payroll_attendanc_sheet', array('Employee_id' => $employee_id, 'Attendancedate' => $Attendancedate))->row();
				$shift_ids=$CI->db->get_where('payroll_work_shift', array('shift_name' => $Shift_name))->row(); 
			...
			//check if shift exists
				...
				$shift_id=$shift_ids->id;
					...
					$CI->db->insert('payroll_work_shift',$ws);
					$shift_id=$CI->db->insert_id();
					$shift_ids = $CI->db->get_where('payroll_work_shift', array('id' => $shift_id))->row();
                    }
				}
				$shiftroster=$CI->db->get_where('payroll_shift_rosters', array('employee_id' => $employee_id, 'Shift_Date' => $Attendancedate,'Shift_id'=>$shift_id))->row();
				//Shift data
				...
				if($shiftroster){
				  ...
				  $CI->db->update('payroll_shift_rosters', $shift_update);
					...
					if(!empty($employee_id)&& !empty($Attendancedate)){
					 ...
					 $CI->db->insert('payroll_shift_rosters', $shiftdata);
					}
				}
				...
				if($result){
				  ...
				  $CI->db->update('payroll_attendanc_sheet', $data_update);
       ...
       if(empty($data))
		{
			 $CI->message->custom_error_msg('admin/payroll/employee/importAttendance', lang('Incorrect_sheet_type'));
		}else
		{
		     $CI->db->trans_start();
             $CI->db->insert_batch('payroll_attendanc_sheet', $data);
             $CI->db->trans_complete();
			if ($CI->db->trans_status() === FALSE)
			{
				  $CI->message->custom_error_msg('admin/payroll/employee/importAttendance', lang('failed_to_import_data'));
			}else{
				$CI->message->custom_success_msg('admin/payroll/employee/importAttendance', lang('import_data_successfully'));
			}
		} 
