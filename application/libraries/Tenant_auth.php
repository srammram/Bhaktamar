...
class Tenant_auth
{
	...
	{
        $tenant = $this->CI->session->userdata('tenant');
        if(!$tenant)
        {
    ...
    function check_access($access, $default_redirect=false, $redirect = false)
    {
        ...
        $this->CI->db->where('id', tenant['id']);
        $this->CI->db->limit(1);
        $result = $this->CI->db->get('tenant_login');
        $result = $result->row();
    ...
    function is_logged_in($redirect = false, $default_redirect = true)
    {
        ...
        if (!$tenant)
        {
            ...
            if(isset($_COOKIE['tenant']))
            {
                ...
                $info = $this->aes256Decrypt(base64_decode($_COOKIE['tenant']));
                $cred = json_decode($info, true);

                if(is_array($cred))
                {
                    if( $this->login_tenant($cred['username'], $cred['password']) )
                    {
    ...
    */
    function login_tenant($username, $password, $remember=false)
    {
        ...
        if (sizeof($result) > 0)
        {
            $tenant = array();
            $tenant['tenant'] = array();
            $tenant['tenant']['id'] = $result['id'];
            $tenant['tenant']['user_role']     = $result['user_role'];
            $tenant['tenant']['firstname']     = $result['firstname'];
			$tenant['tenant']['lastname']      = $result['lastname'];
            $tenant['tenant']['email']         = $result['email'];
            $tenant['tenant']['username']      = $result['username'];
			$tenant['tenant']['building_id']   = $result['project_id'];
			$tenant['tenant']['projectid']     = $result['building_id'];
			$tenant['tenant']['floor']         = $result['floor_id'];
			$tenant['tenant']['unit']         = $result['unit'];
			$tenant['tenant']['venue']        = $result['address'];
			$tenant['tenant']['tenant_type'] = $result['tenant_type'];
			$tenant['tenant']['owner_id']   = $result['Owner_id'];
            if($remember)
            {
            ...
            $this->CI->session->set_userdata($tenant);
            return true;
        }
        else
        {
