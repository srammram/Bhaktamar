function getXMLHTTP() { //fuction to return the xml http object
	...
	function get_employees(str){
		...
		if (str == '') {
			...
			$.ajax({
				...
				success: function(result){
					$("#employee").html("<option value=''>Please Select</option>");
					$("#employee").html("<option value='ALL'>ALL</option>");
					$("#employee").append(result);
				}
			});
	...
	//*********************************************
		//*********************************************
	//     Get Employee Email by employeeid
	//*********************************************

	function get_employee_email(str){

		var employeeId = str;
		if (str == '') {
			$("#email").val("No Email Id Found");
		} else {
			$("#email").html("No Email Id Found");

			var link = getBaseURL();
			$.ajax({
				type: "POST",
				url: link + 'admin/Panel/get_employee_email_by_Employee_id',
				data: { department_id : str, employeeId : employeeId ,csrf_test_name : getCookie('csrf_cookie_name') },
				cache: false,
				success: function(result){
...
//           active HrD Year
//*********************************************
function HrdYear_active(cb) {

	var userId = cb.id;
	var checkstatus = cb.checked;
	var postUrl     = getBaseURL() + 'admin/Office/change_status';
	var csrftoken = getCookie('csrf_cookie_name');
	...
	$.ajax({
		url: postUrl,
		type: "POST",
			data: { userId : userId, checkboxstatus: checkstatus, 'csrf_test_name': csrftoken },
		cache: false,
		success: function(response) {
		 if(response==1)
		   {
			  // alert("Shift Update SuccessFully");
			  $('#msg').html("<div class='alert alert-success'>Year Active Successfully</div>").fadeIn(1000);
			     $('#msg').html("<div class='alert alert-success'>Year Active Successfully</div>").fadeOut(3000);
			 location.reload();
		   }else{
			  // alert("Shift Update Failed");
			  $('#msg').html("<div class='alert alert-danger'>Unable To Activate This Year.</div>").fadeIn(1000);
			  $('#msg').html("<div class='alert alert-danger'>Unable To Activate This Year.</div>").fadeOut(3000);
			   location.reload();
...
function HrdYear_delete(id) {
	var id = id;
	var postUrl     = getBaseURL() + 'admin/Office/delete_Hrd';
	var csrftoken = getCookie('csrf_cookie_name');
	// now upload the file using $.ajax
	$.ajax({
		url: postUrl,
		...
		success: function(response) {
			 ...
			 if(response == 1)
		   {
			  ...
			  $('#msg').html("<div class='alert alert-danger'>Record Has Been Used.</div>").fadeIn(1000);
			  $('#msg').html("<div class='alert alert-danger'>Record Has Been Used.</div>").fadeOut(3000);
