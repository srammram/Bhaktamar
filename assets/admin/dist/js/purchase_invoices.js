...
if (localStorage.getItem('freight')) {
                ...
                if (localStorage.getItem('tax_method')) {
                    localStorage.removeItem('tax_method');
                }
		if (localStorage.getItem('pi_supplier')) {
		     localStorage.removeItem('pi_supplier');
		}
...
$('#pi_discount').focus(function () {
   ...
   $('#add-purchase-invoice .submit-invoice').click(function(e){
...
function loadItems() {
	...
	$bill_disc_per_from = 0;
        $.each(sortedItems, function (){
            var b_item = this;
            $b_net_unitprice = parseFloat(b_item.row.unit_cost)* parseFloat(b_item.row.qty);
            b_item_ds = b_item.row.item_discount_percent;
            
            $b_disType = b_item.row.item_dis_type;

            var itemdiscount = b_item_ds+(($b_disType=="p")?'%':'');
            b_item_ds_amt = calculateDiscount(itemdiscount, $b_net_unitprice); 
            b_item_ds_amt = format2Decimals(b_item_ds_amt);
            $bill_disc_per_from += $b_net_unitprice-b_item_ds_amt;
            
        });
        calculateBillDiscPer($bill_disc_per_from);
...
$markup_selling_price = parseFloat($landingCost) + (parseFloat($landingCost)*(item.row.price_change_value/100));
//alert(item.row.price_change_option)
//alert($markup_selling_price)
item_selling = (item.row.price_change_option=="mark_up")?$markup_selling_price:item_selling;
	    ...
	    $margin_dif = parseFloat(item_selling) - parseFloat($landingCost);
	    ...
	    $row_cnt++;
	    $total_no_items++;
	    $total_no_qty +=parseFloat(item_qty);
        });
        var col = 6;        
	$('#total_no_items').val($total_no_items);
        ...
        var gtotal = ((total + invoice_tax) - order_discount) + feright_chargers_shipping; // + shipping
 ...
 * @returns {Boolean}
 ...
 function add_purchase_invoices_item(item) {
   ...
   //var item_id = item.item_id+'_'+default_store+'_'+item.row.category_id+'_'+item.row.subcategory_id;
    ...
    if (pi_items[item_id]) {
        ...
        item.tax_rate = item.row.tax_rate; //purchase_tax
	item.tax_rate_id = item.row.tax_rate; //purchase_tax
	...
	//console.log('tax_rate='+item.tax_rate+'||tax_rate_id='+item.tax_rate_id+'||tax_val='+item.tax_rate_val)
		...
		pi_items[item_id].row.batch_required = item.row.batch_required;
		pi_items[item_id].row.expiry_date_required = item.row.expiry_date_required;
		pi_items[item_id].row.tax_rate = item.tax_rate; // purchase_tax
...
function calculateBillDiscPer($total){
	if(localStorage.getItem('bill_disc')==null){
		localStorage.setItem('bill_disc',0);
	}
    bill_disc =localStorage.getItem('bill_disc');// $('.bill_disc').val();
	//alert(localStorage.getItem('bill_disc'));
	//alert(bill_disc);
    if (bill_disc.indexOf("%") !== -1) {
        bill_disc_per = bill_disc;
    }else{
        //$bil_dis_from = parseFloat($('#final_gross_amt').val())-parseFloat( $('#item_disc').val());
        //if (bill_disc>$bil_dis_from) {
        //    bootbox.alert('Discount should not be greater than net amount');
        //    $('.bill_disc').val($old_bill_disvalue);
        //    return false;
        //}
        $bill_disc_per = (bill_disc*100)/$total;
        bill_disc_per = $bill_disc_per+('%');
    }
