...
$(document).ready(function() {
    ...
    $('#search-product').select2({
       minimumInputLength: 1,
       ajax: {
        url: site.base_url+"products/product_suggestions",
        dataType: 'json',
        quietMillis: 15,
        data: function (term, page) {
            return {
    ...
    $(document).on('click', '.po-delete', function(e) {
        ...
        $.ajax({type: "get", url: link, dataType: 'json',
            success: function(data) {
                ...
                else { addAlert(data.msg, 'success');row.remove(); if(oTable != '') { oTable.fnDraw(); } }
            },
...
function formatDecimal(x, d) {
   ...
   // return parseFloat(accounting.formatNumber(x, d, '', '.'));
   return parseFloat(formatNumber(x, d, '', '.')).toFixed(d);
}
...
function recipe_stock(x) {
    ...
    return '<a href="'+site.base_url+'products/stock/'+ y +'" data-toggle="modal" data-target="#myModal" data-backdrop="static" data-keyboard="false">stock<a/>';
...
if (isWarehouse=='1') {
   function user_status(x) {
    ...
    var y = x.split("__");
    return y[0] == 1 ?
    '<i class="fa fa-check"></i> '+lang['active']+'</span>' :
    '<i class="fa fa-times"></i> '+lang['inactive']+'</span>';
    } 
}
*/
...
function fixAddItemnTotals() {
    ...
    $(window).scroll(function() {
        ...
        if (windowpos >= aiTop) {
            ai.addClass("stick").css('width', ai.parent('form').width()).css('zIndex',999);
        ...
        if (windowpos <= ($(document).height() - $(window).height() - 120)) {
            bt.css('position', 'fixed').css('bottom', 0).css('width', bt.parent('form').width()).css('zIndex', 999);
        } else {
...
$(document).ready(function(){
    $('#search-products-submitt').click(function(e){
        e.preventDefault();
        $formData = $('#search-products-form').serialize();
        $.ajax({
            url  : site.base_url+"products/search_products",
            type:'post',
            dataType:'json',
            data:$formData,
            success:function(res){
                ...
                if (res) {
                    ...
                    $.each(res,function(n,v){
                        $html += '<tr  class="p-container">';
                        $html += '<td>'+v.category_name+'</td>';
                        $html += '<td>'+v.code+'</td>';
                        $html += '<td>'+v.name+'</td>';
                        $html += '<td><span class="p-selling-price-label">'+v.price+'</span><input type="hidden" name="selling_price['+v.id+']" class="p-selling-price" value="'+v.price+'"></td>';
                        $html += '<td><span class="p-mrp-label">'+v.mrp+'</span><input type="hidden" name="mrp['+v.id+']" class="p-mrp" value="'+v.mrp+'"></td>';
                        $html += '<td><span class="p-purchase-cost-label">'+v.cost+'</span><input type="hidden" name="purchase_cost['+v.id+']" class="p-purchase-cost" value="'+v.cost+'"></td>';
                        $html +='</tr>';
                    });
    ...
    $('#apply-new-price').click(function(e){
        e.preventDefault();
        $('#update-with').css('border',"1px solid #ccc");
        if ($('#update-with').val()=='') {
            $('#update-with').css('border',"1px solid #f00");
            return false;
        }
        $change_price = $('#change-price-type').val();
        $price_type = $("#select-price-type").val();
        $plus_minus = $('.plus_minus:checked').val();
        $update_with = $('#update-with').val();
        
        $('#product-list tbody tr').each(function(n,v){
            $tr = $(this);
            if ($price_type=="mrp") {
               
               $update_from = $tr.find('.p-mrp').val();
            }else if($price_type=="selling_price"){
                $update_from =  $tr.find('.p-ex-selling-price').val();
            }else{
                $update_from =  $tr.find('.p-purchase-cost').val();
            }
            
            if ($update_with.indexOf("%")!=-1) {
             $price_per = $update_with.replace('%','');
             $price = $update_from *($price_per/100);
             ...
             if ($plus_minus=='+') {
                 $new_price = parseFloat($update_from) + parseFloat($price);
             }else{
                 $new_price = parseFloat($update_from) - parseFloat($price);
             }
            $tr.find('.p-selling-price').val($new_price);
            $tr.find('.p-selling-price-label').text($new_price);
...
function calculateExclusiveTax($tax_rate,$total){
    return formatDecimal($total*(parseFloat($tax_rate)/parseFloat(100)));
}
function calculateInclusiveTax($tax_rate,$total){
    $tax_amt =  ($total*($tax_rate))/(parseFloat(100)+parseFloat($tax_rate));
    return formatDecimal($tax_amt);
}
